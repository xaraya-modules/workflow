...
if (!function_exists('galaxia_execute_activity')) {
    function galaxia_execute_activity($activityId = 0, $iid = 0, $auto = 1)
    {
        ...
        if (!$fp) {
            trigger_error(\xarMLS::translate("Fatal error: cannot execute automatic activity $activityId"), E_USER_WARNING);
            die;
        }
        ...
        /*
        if(!empty($data)) {
            trigger_error(\xarMLS::translate("Fatal error: automatic activity produced some output:$data"), E_USER_WARNING);
        }
        */
        fclose($fp);
    }
